...
const rivalAction = async (requester, action, userDataContext) => {
  var endPoint;
  switch(action) {
...
const storeNewRivals = async (requester, action, userDataContext) => {
...
const acceptRivalNewState = (requester, prevState) => {
...
const rejectRivalNewState = (requester, prevState) => {
  const newFollowerRequests = prevState.followerRequests.filter(user => {
    return user._id !== requester._id
  })
  return {
...
const removeRivalNewState = (requester, prevState) => {
...
module.exports = {rivalAction, storeNewRivals}